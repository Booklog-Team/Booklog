...
const STATUS_CLASS = { reading: "status-reading", want: "status-want", done: "status-done" };
...
function CoverImage({ src, alt, className }) {
  ...
  if (!src || failed) {
    return (
      <div className={`${className} bg-secondary/70 flex items-center justify-center`}>
...
export default function BookCard({ book, variant = "compact" }) {
  ...
  const progress =
    totalPages && currentPage ? Math.round((currentPage / totalPages) * 100) : null;
    ...
    return (
      <Link to={`/book/${id}`}>
        <div className="book-card p-4 flex gap-4 active:scale-[0.99] transition-transform cursor-pointer">
          <CoverImage src={cover} alt={title} className="w-16 h-24 object-cover rounded-lg shadow-sm flex-shrink-0" />

          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2 mb-0.5">
              <h3 className="font-semibold text-sm leading-snug line-clamp-2">{title}</h3>
              {book.status && (
                <span className={`tag-pill ${STATUS_CLASS[book.status]} flex-shrink-0`}>
                  {STATUS_LABEL[book.status]}
                </span>
              )}
            </div>
            <p className="text-xs text-muted-foreground line-clamp-1 mb-1">{author}</p>
            {(publisher || year || pageCount) && (
              <p className="text-[11px] text-muted-foreground/70 line-clamp-1 mb-1">
                {[publisher, year, pageCount ? `${pageCount}p` : null].filter(Boolean).join(" · ")}
              </p>
            )}
            {(category || rating > 0) && (
              <div className="flex items-center gap-2 mb-2">
                ...
                {rating > 0 && (
                  <span className="flex items-center gap-0.5">
                    ...
                    <span className="text-[11px] font-medium text-amber-600">{rating.toFixed(1)}</span>
                  </span>
                )}
              </div>
            )}
            ...
            {book.memo && (
              <p className="text-xs text-muted-foreground mt-2 line-clamp-1 italic">"{book.memo}"</p>
            )}
          </div>
        </div>
      </Link>
    ...
    <Link to={`/book/${id}`}>
      <div className="flex flex-col gap-2 active:scale-[0.97] transition-transform cursor-pointer">
        <div className="relative">
          <CoverImage src={cover} alt={title} className="w-full aspect-[2/3] object-cover rounded-lg shadow-md" />
          {book.status && (
            <span className={`tag-pill ${STATUS_CLASS[book.status]} absolute top-2 left-2 shadow-sm`}>
              {STATUS_LABEL[book.status]}
            </span>
          )}
          {progress !== null && (
        ...
        <div>
          <p className="text-xs font-semibold line-clamp-2 leading-snug">{title}</p>
