        ...
        <div className="book-card p-4 flex gap-4 active:scale-[0.99] transition-transform">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2 mb-1">
              ...
              <span
                className={`tag-pill ${STATUS_CLASS[book.status || "unregistered"]} flex-shrink-0`}
              >
                {STATUS_LABEL[book.status || "unregistered"]}
              </span>
            </div>
    ...
    <Link to={`/book/${book.id}`}>
      <div className="flex flex-col gap-2 active:scale-[0.97] transition-transform">
        <div className="relative">
          ...
          />
          <span
            className={`tag-pill ${STATUS_CLASS[book.status || "unregistered"]} absolute top-2 left-2 shadow-sm`}
          >
            {STATUS_LABEL[book.status || "unregistered"]}
          </span>
          {progress !== null && (
