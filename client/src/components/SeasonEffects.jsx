 ...
 */
export default function SeasonEffects({ layer = "foreground" }) {
  ...
  return (
    <AnimatePresence>
      {layer === "background" && theme === "glass" && (
        <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden bg-[#CACACA]">
          {/* 🍏 유리: 오로라 메쉬 그라디언트 (Aurora Mesh Gradient) */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            ...
            <motion.div
              ...
              className="absolute -top-48 -left-48 w-[600px] h-[600px] rounded-full bg-[#007AFF]/40 blur-[80px]"
            />
            {/* Blob 2: Purple */}
            <motion.div
              animate={{ x: [20, -20, 20], y: [20, -20, 20] }}
              transition={{ duration: 10, repeat: Infinity, ease: "easeInOut" }}
              className="absolute -bottom-48 -right-48 w-[500px] h-[500px] rounded-full bg-[#5856D6]/40 blur-[80px]"
            />
            {/* Blob 3: Pink */}
            <motion.div
              animate={{ x: [-30, 30, -30], y: [30, -30, 30] }}
              transition={{ duration: 12, repeat: Infinity, ease: "easeInOut" }}
              className="absolute top-1/3 right-[10%] w-[400px] h-[400px] rounded-full bg-[#FF2D55]/25 blur-[80px]"
            />
        ...
        <div className="fixed inset-0 pointer-events-none z-[120] overflow-hidden">
          ...
          {theme === "winter" && (
            ...
            >
              <svg className="w-full h-24 overflow-visible" preserveAspectRatio="none">
                <path d="M0,10 Q100,60 200,10 T400,10 T600,10 T800,10 T1000,10 T1200,10 T1440,10" fill="none" stroke="#2C3E50" strokeWidth="1.5" className="opacity-70" />
                {[...Array(15)].map((_, i) => {
                  const x = (i * 100) + 30;
                  ...
                  return (
                    <g key={i}>
                      ...
                      <motion.circle
                        cx={x} cy={y + 2} r="6"
                        animate={{ opacity: [0.5, 1, 0.5], scale: [0.95, 1.15, 0.95], filter: ["drop-shadow(0 0 2px currentColor)", "drop-shadow(0 0 15px currentColor)", "drop-shadow(0 0 2px currentColor)"] }}
          ...
          {effects.map((item) => (
            <motion.div
              ...
              className="absolute"
            >
              {item.type === "petal" && <div className="bg-pink-300/60 rounded-full blur-[0.5px]" style={{ width: item.size, height: item.size / 1.5, borderRadius: "100% 20% 100% 20%" }} />}
              {item.type === "bubble" && <div className="border border-white/30 bg-white/10 rounded-full" style={{ width: item.size, height: item.size }} />}
              {item.type === "leaf" && <Leaf size={item.size} className="text-orange-700/40" />}
              {item.type === "xmas" && <item.icon size={item.size} className={`${item.color} opacity-60 drop-shadow-sm`} />}
            </motion.div>
          ))}
        </div>
      )}
